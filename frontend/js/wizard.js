  ...
  function readCurrentPageInputs() {
    ...
    pages[pageIdx].questions.forEach(q => {
      ...
      if (fd.has(f)) {
        ...
        if (el)
        {
          // If this feature already has a Yes/No, store the rating separately
          if (answers[f] && typeof answers[f] === 'string' && ['yes','no'].includes(answers[f].toLowerCase())) {
            answers[f + '_rating'] = el.value;
          } else {
            answers[f] = el.value;
          }
        }
  ...
  function renderQuestion(q) {
    ...
    if (q.type === 'radio') {
      ...
      input.addEventListener('input', () => {
        ...
        if (answers[nameAttr] && String(answers[nameAttr]).toLowerCase() === 'yes') {
          answers[nameAttr] = (Number(input.value) / 10).toString();  // store as scaled string
        } else {
          answers[nameAttr] = input.value;
        }
      });      
